...
parser.add_argument('-e', '--img_extension', type=str, required=True, help='Image extension.')
parser.add_argument('--out', type=str, required=False, help='Output parent folder to build the tree. Created if '
                                                           'does not exist')
...
#build_train_val_folders(video_path, output_parent_folder, item_name='frame', extension='png', val_set_size=args.val_size, min_n_frame=args.mf)
build_val_folders(parent_folder=args.vp, extension=args.img_extension, labels_folder=args.labels_folder)