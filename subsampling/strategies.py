...
def thresholding_object_count(
    ...
    **kwargs,
    ...
    threshold = np.percentile(
        [count for _, count in warmup_set], 100 * (1 - sampling_rate)
    )

    # Filtering images based on the object count
    top_count_images = [
        (img, count)
        for img, count in object_counts[warmup_length:]
        if count > threshold
    ]
...
def bernoulli_least_confidence(
    image_labels_path: str,
    n: int = DEFAULT_SUB_SAMPLE,
    aggregation_function: str = "max",
    strategy="logistic",
    ...
    **kwargs,
    ...
    low_confidence_images = [
        (img, conf)
        for i, (img, conf) in enumerate(confidences)
        if np.random.binomial(1, probabilities[i])
    ]
...
def strategy_fixed_interval(
    image_folder_path: str, imgExtension: str, val_size: int, n: int = 1, **kwargs
) -> list:
    """
    :param image_folder_path: path to the bank image folder
    :param n: number of frames to select
...
def strategy_dense_optical_difference(
    image_folder_path: str,
    imgExtension: str,
    val_size: int,
    n: int = DEFAULT_SUB_SAMPLE,
    ...
    **kwargs,
    ...
    output_list = [0]
    frame1 = cv2.imread(
        os.path.join(image_folder_path, path_list[output_list[-1]] + default_ext)
    )
    ...
    for e, i in enumerate(data_to_process):
        ...
        result = optical_flow_compare(
            frame1, frame2, mask, fix_outliers=True, fixer_precision=fixer_precision
        )
        ...
        if mag_sum > difference_ratio:
            ...
            frame1 = cv2.imread(
                os.path.join(
                    image_folder_path, path_list[output_list[-1]] + default_ext
                )
            )

        print(
            f"\rProcessed [{e + 1}/{len(data_to_process) + 1}] (Default jumping by {jump} frames), Selected [{len(output_list)}/{n}]",
            end="",
        )
    ...
    if len(output_list_processed) >= n:
        ...
        print(
            f"Selected [{len(output_list)}/{n}] Through Movement Analysis (Optical Flow Difference), Will fill the rest with other frames..."
        )
        output_list_processed = fill_in_between(path_list, output_list_processed, n)
    else:
        print(
            f"Selected [{len(output_list)}/{n}] Through Movement Analysis (Optical Flow Difference). Will not fill the rest since filling is disabled."
        )
        return output_list_processed


def generate_path_to_frequencies(
    image_folder_path: str, bank_folder_path: str, imgExtension: str
):
    dic = {}
    count1 = 0
    for file in glob.glob(os.path.join(image_folder_path, "*." + imgExtension)):
        ...
        dic[file.replace(imgExtension, "")] = [
            np.absolute(sumFrequency_original),
            np.absolute(sumFrequency_removed),
        ]
        count1 = count1 + 1
    sumFrequencyDataset = pd.DataFrame.from_dict(
        dic, orient="index", columns=["frequency", "frequency_filtered"]
    )
...
def diversify_classes(counts, n: int = DEFAULT_SUB_SAMPLE):
    ...
    class_with_zeros = np.where(dis == 0.0)
    ...
    assert np.all(
        (dis >= 0) | np.all(dis < 0)
    ), "Issue with the distribution, it contains negative assignation"
...
def strategy_frequency(
    ...
    **kwargs
    ...
    if exists(os.path.join(bank_folder_path, "frequencies.txt")):
        ...
        path_to_frequencies = generate_path_to_frequencies(
            image_folder_path, bank_folder_path, imgExtension
        )
    ...
    groups = (
        10  # Hyperparameter of the method. It gives the number of expected cluster.
    )
    (counts, bins) = np.histogram(
        sortedDataFiltered.get("frequency_filtered"), groups
    )  # Bins are the edges of the different cluster
    ...
    condlist = [None] * len(
        counts
    )  # Initalization of the condition list to assign the group
    for l in range(0, len(bins) - 1):
        condlist[l] = (sortedDataFiltered.frequency_filtered >= bins[l]) & (
            sortedDataFiltered.frequency_filtered <= bins[l + 1]
        )  # @Fixme, In this version the borders's class will be outwritten. We don't care as the method is not sensitive to borders.

    # STEP 3 - Clustering
    condarray = np.array(
        condlist
    )  # bool representation where each row is a condition and each column is a row of the df
    ...
    for i in range(0, len(dis)):
        to_select = sortedDataFiltered[sortedDataFiltered["group"] == i].head(
            int(dis[i])
        )
...
def strategy_flow_interval_mix(
    image_folder_path: str,
    imgExtension: str,
    val_size: int,
    n: int = DEFAULT_SUB_SAMPLE,
    movement_percent: int = 90,
    difference_ratio: float = 4.0,
    **kwargs,
) -> list:
    """
    ...
    output_list = [movement_frames[s] for s in movement_indices] + [
